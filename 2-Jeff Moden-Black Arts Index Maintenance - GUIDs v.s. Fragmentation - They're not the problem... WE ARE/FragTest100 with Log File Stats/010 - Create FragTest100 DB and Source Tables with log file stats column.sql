...
/**********************************************************************************************************************
 Purpose:
 1. Create the "FragTest100" database.
...
-----------------------------------------------------------------------------------------------------------------------
 ...
 2. It takes this code approximately 52 seconds to execute on my laptop running SQL Server 2017 Developer's Edition.
        ...
        2 TB NVME SSD ("Disk")
 3. The final size of the database this code creates is 103GB.
        The MDF file is 100GB.
        The LDF file is  3GB.
 ...
 6. The name of the database is "FragTest100"
----------------------------------------------------------------------------------------------------------------------- 
 Revision History:
 ...
 Rev 05 - 04 Mar 2020 - Jeff Moden
        - Modify a copy of the FragTest10 code to create this FragTest100 code.
...
RAISERROR('    
--=====================================================================================================================
--      Drop and create the "FragTest100" Database
--=====================================================================================================================
',0,0) WITH NOWAIT
;
...
--===== If the database exists, drop it.
     ...
     IF DB_ID('FragTest100') IS NOT NULL
  BEGIN 
        RAISERROR('Dropping existing database...',0,0) WITH NOWAIT
        ;
        --===== Kick everyone and everything out of the database.
          ALTER DATABASE FragTest100 SET SINGLE_USER WITH ROLLBACK IMMEDIATE
        ;
        --===== Drop the database
           DROP DATABASE FragTest100
        ;
    END
   ELSE RAISERROR('Database did not exist.',0,0) WITH NOWAIT
;
--===== Create the database using only the defaults and set to the FULL Recovery Model.
RAISERROR('Creating the database...',0,0) WITH NOWAIT
;
 CREATE DATABASE FragTest100;
  ALTER DATABASE FragTest100 SET RECOVERY FULL WITH NO_WAIT
;
...
;
  ALTER DATABASE FragTest100 MODIFY FILE (NAME = N'FragTest100'    , SIZE = 100000MB, FILEGROWTH = 100MB);
  ALTER DATABASE FragTest100 MODIFY FILE (NAME = N'FragTest100_log', SIZE =   3000MB, FILEGROWTH = 100MB)
;
GO
RAISERROR('
--=====================================================================================================================
--      Create the two source tables (36,500,000 GUIDs each)
--=====================================================================================================================
',0,0) WITH NOWAIT
;
...
--      Change to the BULK_LOGGED recovery model so Index Rebuilds run quickly thanks to "Minimal Logging".
--      Can''t help Reorganize no matter what we do because it''s fully logged all the time.
--=====================================================================================================================
',0,0) WITH NOWAIT
;
        CHECKPOINT;
 BACKUP DATABASE FragTest100 TO DISK = 'NUL';
 BACKUP LOG      FragTest100 TO DISK = 'NUL';
  ALTER DATABASE FragTest100 SET RECOVERY BULK_LOGGED WITH NO_WAIT;
GO
RAISERROR('
--=====================================================================================================================
