/**********************************************************************************************************************
 ...
 The only difference between this script and the LTDD_* series test code is that this series runs in the FULL
 Recovery Model AND does rebuilds WITH (ONLINE = ON).
...
-----------------------------------------------------------------------------------------------------------------------
 Operator Notes:
 1. It takes this code approximately 01:31:00 (hh:mm:ss) to execute on my laptop running SQL Server 2008 
    Developer's Edition.
...
----------------------------------------------------------------------------------------------------------------------- 
 Revision History:
 Rev 00 - 02 Jan 2020 - Jeff Moden
        - Initial creation and use.
 Rev 01 - 02 Jul 2021 - Jeff Moden
        - Finalize changes to include the new test proc that records transaction log file stats.
**********************************************************************************************************************/
--=====================================================================================================================
--      Presets
--=====================================================================================================================
--===== Ensure that the database is in the FULL Recovery Model
  ALTER DATABASE [FragTest10] SET RECOVERY FULL WITH NO_WAIT
;
--===== Local Variables
...
--===== Constants: These settings affect ALL tests the same way
 SELECT  @TestDays          = 365
        ...
        ,@pDF_RebuildOnline ='Yes' -- Use ONLINE rebuilds?  Yes or No.
;
...
--===== Low Threshold w/Fill Factor = 90
   ...
   EXEC dbo.PopulateGuidTestAdaptive_FragTest10
         @pTestName         = 'LTDDFullOnline_FF090'
        ,@pTestDays         = @TestDays     
        ...
        ,@pDF_Days          = @DF_Days
        ,@pDF_RebuildOnline = @pDF_RebuildOnline
;
--===== Rename the table to keep it for later experiments
     IF OBJECT_ID('dbo.LTDDFullOnline_FF090','U') IS NOT NULL
        DROP TABLE dbo.LTDDFullOnline_FF090
;
   EXEC sp_rename 'dbo.GuidTest','LTDDFullOnline_FF090'
;
-----------------------------------------------------------------------------------------------------------------------
...
--===== Low Threshold w/Fill Factor = 80
   ...
   DBCC FREEPROCCACHE;
   EXEC dbo.PopulateGuidTestAdaptive_FragTest10
         @pTestName         = 'LTDDFullOnline_FF080'
        ,@pTestDays         = @TestDays     
        ,@pHoursPerDay      = @HoursPerDay  
        ,@pRowsPerHour      = @RowsPerHour  
        ,@pFillFactor       = 80   
...
;
--===== Rename the table to keep it for later experiments
     IF OBJECT_ID('dbo.LTDDFullOnline_FF080','U') IS NOT NULL
        DROP TABLE dbo.LTDDFullOnline_FF080
;
   EXEC sp_rename 'dbo.GuidTest','LTDDFullOnline_FF080'
;
-----------------------------------------------------------------------------------------------------------------------
--===== Low Threshold w/Fill Factor = 70
   ...
   DBCC FREEPROCCACHE;
   EXEC dbo.PopulateGuidTestAdaptive_FragTest10
         @pTestName         = 'LTDDFullOnline_FF070'
        ,@pTestDays         = @TestDays     
        ,@pHoursPerDay      = @HoursPerDay  
        ,@pRowsPerHour      = @RowsPerHour  
        ,@pFillFactor       = 70   
...
;
--===== Rename the table to keep it for later experiments
     IF OBJECT_ID('dbo.LTDDFullOnline_FF070','U') IS NOT NULL
        DROP TABLE dbo.LTDDFullOnline_FF070
;
   EXEC sp_rename 'dbo.GuidTest','LTDDFullOnline_FF070'
;
-----------------------------------------------------------------------------------------------------------------------
--=====================================================================================================================
...
--===================================================================================================================== 
 SELECT  [DayHour]
  ...
  WHERE TestName LIKE 'LTDDFullOnline%'
...
--      Run Complete
--===================================================================================================================== 
--===== Make sure that everything has been committed to disk. 
        CHECKPOINT
;
--===== Change the Recovery Model back to BULK LOGGED.
