/**********************************************************************************************************************
 ...
 Defrag Weekly with Low Threshold Rebuilds (1%): Test for whether to defrag or not is executed weekly.
...
-----------------------------------------------------------------------------------------------------------------------
 Operator Notes:
 1. It takes this code approximately 00:32:51 (hh:mm:ss) to execute on my laptop running SQL Server 2008 
    Developer's Edition.
...
--===== Constants: These settings affect ALL tests the same way
 SELECT  @TestDays          = 365
        ...
        ,@DF_Days           = 7    --Number of days to skip between defrags
;
...
--===== Rename the table to keep it for later experiments
     IF OBJECT_ID('dbo.LTWK_FF090','U') IS NOT NULL
        DROP TABLE dbo.LTWK_FF090
;
   EXEC sp_rename 'dbo.GuidTest','LTWK_FF090'
;
-----------------------------------------------------------------------------------------------------------------------
...
;
--===== Rename the table to keep it for later experiments
     IF OBJECT_ID('dbo.LTWK_FF080','U') IS NOT NULL
        DROP TABLE dbo.LTWK_FF080
;
   EXEC sp_rename 'dbo.GuidTest','LTWK_FF080'
;
-----------------------------------------------------------------------------------------------------------------------
...
;
--===== Rename the table to keep it for later experiments
     IF OBJECT_ID('dbo.LTWK_FF070','U') IS NOT NULL
        DROP TABLE dbo.LTWK_FF070
;
   EXEC sp_rename 'dbo.GuidTest','LTWK_FF070'
;
-----------------------------------------------------------------------------------------------------------------------
--=====================================================================================================================
